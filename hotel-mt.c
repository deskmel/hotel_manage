...
PNode addvisitors(PNode p,visitor s,int i)
{
...
void alter_room(PNode head,int room_index,room s)
{
	...
	char bf[2]="";
	itoa(s.breakfast,bf,10);
	char room_bf[30]="breakfast=";
	strcat(room_bf,bf);
	
	char night_[4]="";
	itoa(s.nights,night_,10);
	char night[30]="night=";
	strcat(night,night_);
...
PNode initia_room(PNode head,int room_index,const char *type="dorm")
{
	char room_in[30]={"[room "};
	char index[4];
	itoa(room_index,index,10);
...
PNode initialize(int floor=4,int num=20)
{
...
void sort_byname(PNode head)
{
	...
	while (p!=NULL and p->next->next->next!=NULL)
	{	
